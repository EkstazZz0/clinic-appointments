...
from datatest import appointment_for_response, appointment_description, appointment_time, appointment_error_description
...
def test_2_create_error_bad_description_appointment(client: TestClient, prepared_doctor):
    ...
    assert response.json().get("detail")[0].get('msg') == 'String should have at most 3000 characters'
...
def test_4_create_error_same_doctor_and_time_appointment(client: TestClient, prepared_doctor):
    client.post("/appointments", json=appointment_for_response)
    response = client.post("/appointments", json=appointment_for_response)
    assert response.status_code == 422
    assert response.json() == {'detail': "Doctor can't accept several appointments at the same time."}
...
def test_6_get_error_non_existent_appointment(client: TestClient):
    response = client.get("/appointments/1")
    assert response.status_code == 404
    assert response.json() == {"detail": "Appointment with id 1 was not found in database."}
