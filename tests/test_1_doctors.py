...
from datatest import doctor_for_response, doctor_error_create, doctor_error_create_response
...
def test_4_get_error_doctor(client: TestClient):
    for _ in range(10):
        ...
        assert response.json() == {'detail': f'Doctor with id {doctor_id} was not found in database.'}
